        ...
        {filtered.map((c, i) => (
          <motion.div key={c.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-card rounded-xl border p-5 hover:shadow-md transition-shadow cursor-pointer" onClick={() => setSelectedClient(c.id)}>
            <div className="flex items-start justify-between mb-3">
              ...
              <div className={`text-2xl font-bold font-heading ${getScoreColor(c.score)}`}>{c.score}</div>
            </div>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <div className={`text-4xl font-bold font-heading ${getScoreColor(client.score)}`}>{client.score}</div>
                <div>
              ...
              <div className="grid grid-cols-3 gap-3 p-3 bg-muted rounded-lg text-center text-sm">
                ...
                <div><p className="text-lg font-bold font-heading">{client.score}</p><p className="text-muted-foreground text-xs">Score</p></div>
              </div>
